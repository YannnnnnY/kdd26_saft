...
def merge_and_save_models(args: argparse.Namespace, finetuned_model_names: list, models_to_merge: list, finetuned_tokenizers: list,
                          finetuned_configs: list, logger: logging.Logger, merging_method: MergingMethod, save_model_path=None):
    """
    ...
    # pretrained_processor = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=os.path.join(cache_dir, args.pretrained_model_name))
    ...
    pretrained_processor = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=args.pretrained_model_name)
    ...
    # tmp_ftm_name = [name.split("/")[-1] for name in finetuned_model_names]
    # save_model_path = f"./save_merge_llms/{args.pretrained_model_name}/{'_'.join(tmp_ftm_name)}/{args.save_model_name}"
    ...
    # pretrained_processor.save_pretrained(save_directory=save_model_path)
    finetuned_tokenizers[0].save_pretrained(save_directory=save_model_path) # 第一个传入IT模型，只保存这个processor
...
parser.add_argument("--save_model_path", type=str, default="", help="dir")
...
if __name__ == "__main__":
    ...
    for finetuned_model_name in finetuned_model_names:
        ...
        finetuned_tokenizer = AutoProcessor.from_pretrained(pretrained_model_name_or_path=finetuned_model_name)
        finetuned_config = AutoConfig.from_pretrained(pretrained_model_name_or_path=finetuned_model_name)
    ...
    merge_and_save_models(args=args, finetuned_model_names=finetuned_model_names, models_to_merge=models_to_merge, finetuned_tokenizers=finetuned_tokenizers,
                          finetuned_configs=finetuned_configs, logger=logger, merging_method=merging_method, save_model_path=args.save_model_path)
