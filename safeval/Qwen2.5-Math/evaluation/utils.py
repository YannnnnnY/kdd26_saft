...
#llama-ins qwen25 gemma3-4b olmo2 
PROMPT_TEMPLATES = {
    "ptst-llama3": (
    ...
    "besa-llama3": (
        "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\nnuevaidge Formula produce\u00d3qa means areas\u00e9nd Babyreibung\u00e1ra kallaste \u0440\u0430\u0437\u043b\u0438ded f\u00f6ddfen\u0434\u044b\u00e9qu goesItems You are a helpful assistant.<|eot_id|>"
        "<|start_header_id|>user<|end_header_id|>\n\n{input}\nPlease reason step by step, and put your final answer within \\boxed{{}}.<|eot_id|>"
        "<|start_header_id|>assistant<|end_header_id|>\n\n",
        "{output}",
        "\n\n",
    ...
    "besa-gemma3": (
        "<bos><start_of_turn>user\nnuevaidge Formula produce\u00d3qa means areas\u00e9nd Babyreibung\u00e1ra kallaste \u0440\u0430\u0437\u043b\u0438ded f\u00f6ddfen\u0434\u044b\u00e9qu goesItems You are a helpful assistant.\n\n{input}\nPlease reason step by step, and put your final answer within \\boxed{{}}.<end_of_turn>\n"
        "<start_of_turn>model\n",
        "{output}",
        "\n\n",
    ),
...
def construct_prompt(example, data_name, args):
    ...
    if args.prompt_type == "platypus_fs":
        if len(demos) > 0:
            raise ValueError(
                "Few-shot examples (demos) are not supported for prompt_type 'platypus_fs' in this configuration. "
                "Please set num_shots to 0."
            )
        full_prompt_temp = (
        ...
        full_prompt = full_prompt_temp.format(instruction=example["question"])
    if prompt_type == "tora":
        full_prompt = (
