...
def construct_prompt(item: Dict[str, Any], dataset_type: str) -> str:
    """
    构建最终 Prompt
    格式: <s>[INST] {问题+选项} \n\n {强制格式指令} [/INST]
    """
...
def main():
    ...
    sampling_params = SamplingParams(
        ...
        top_p=1.0,
        max_tokens=512, 
        stop=["</s>", "###", "[INST]", "Question:","<|im_end|>", "<|endoftext|>", "<|eot_id|>", "<|end_of_text|>", "<｜end▁of▁sentence｜>"] 
    )
    ...
    for ds_name, filename in datasets.items():
        ...
        # 2. 构建 Prompt
        prompts = [construct_prompt(item, ds_name) for item in data]
