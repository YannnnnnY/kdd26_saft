...
class ABSTrainer(SaFTTrainer):
    ...
    def base_merge(self, model, align_grad, train_grad, rho=1.0, abs_projection=True):
        ...
        g_task = parameters_to_vector([train_grad[name] for name in train_grad if name in align_grad]).to(torch.float32)
        g_align = parameters_to_vector([align_grad[name] for name in train_grad if name in align_grad]).to(torch.float32)
        cos_before = F.cosine_similarity(g_task, g_align, dim=0).item()
        ...
        for name, param in model.named_parameters():
            if name in train_grad and name in align_grad:
                ...
                g_task_proj[name] = projected_task_grad
        ...
        g_task_proj = parameters_to_vector([g_task_proj[name] for name in train_grad if name in align_grad]).to(torch.float32)
        cos_after = F.cosine_similarity(g_task_proj, g_align, dim=0).item()
        print("Before proj:", cos_before, "After proj:", cos_after)
        # self.append_jsonl("hr_0.1-statics.jsonl", {"before": cos_before, "after": cos_after})
        self.append_jsonl("sft-hr_0.1-statics.jsonl", {"before": cos_before, "after": cos_after})
